...
const Drawer = styled.div<{ isOpen: boolean }>`
  ...
  z-index: 1000; /* Ensure the drawer is above other elements */
`;
...
const Sidebar = (props: Props) => {
  ...
  return (
    ...
    {/* image ,  setting , logout block  */}
      ...
      <ImageContainer style={{ display: 'flex', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', columnGap: 12 }}>
          ...
          <MainText style={{ alignItems: 'center', paddingTop: '5px' }}>
            <p style={{ fontWeight: 300, color: 'grey ', fontSize: 14 }}>Good Morning,</p>
            <p style={{ fontWeight: 500, fontSize: 18 }}> Gaurab Thapa</p>
          </MainText>
        </div>
        <ButtonContainer style={{ display: 'flex', gap: 15, marginTop: '20px', justifyContent: 'space-between', justifySelf: 'end' }}>
          {/* Power off icon */}
          <div className="grid w-8 h-8 card bg-base-300 place-items-center" style={{ borderRadius: '8px' }}>
          <svg fill="#000000" width="20px" height="20px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 21c4.411 0 8-3.589 8-8 0-3.35-2.072-6.221-5-7.411v2.223A6 6 0 0 1 18 13c0 3.309-2.691 6-6 6s-6-2.691-6-6a5.999 5.999 0 0 1 3-5.188V5.589C6.072 6.779 4 9.65 4 13c0 4.411 3.589 8 8 8z"/><path d="M11 2h2v10h-2z"/></svg>
          </div>
          {/* Settings icon */}
        {!isDrawerOpen && <div className="grid w-8 h-8 card bg-base-300 place-items-center" style={{ borderRadius: '8px' ,cursor:'pointer' }} onClick={() => setIsDrawerOpen(!isDrawerOpen)}>
    ...
    {/* Freq Used or Most Used  */}
      ...
      <Drawer isOpen={isDrawerOpen} ref={drawerRef}>
